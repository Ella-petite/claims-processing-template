...
function renderMetrics() {
  const open = state.claims.filter(claim => !['Paid', 'Rejected'].includes(claim.status)).length;
  const review = state.claims.filter(claim => claim.status === 'UnderReview').length;
  const paid = state.claims.filter(claim => claim.status === 'Paid').reduce((sum, claim) => sum + claim.amount, 0);
...
function renderDetail() {
  ...
  const checks = [historyNotes.includes('customer validated'), historyNotes.includes('policy validated'), historyNotes.includes('manual review required') || historyNotes.includes('fraud'), claim.status === 'UnderReview', ['PaymentPending', 'Paid'].includes(claim.status), claim.status === 'Paid'];
  const stageMarkup = stages.map((stage, index) => `<div class="stage ${checks[index] && index !== 3 ? 'done' : ''} ${index === 3 && claim.status === 'UnderReview' ? 'current' : ''}"><span class="stage-dot">${checks[index] && !(index === 3 && claim.status === 'UnderReview') ? '✓' : index === 3 && claim.status === 'UnderReview' ? '!' : ''}</span><span class="stage-name">${stage}</span></div>`).join('');
  detail.innerHTML = `
    <div class="detail-header"><div class="detail-title"><div><h2>${claim.claimReference}</h2><p>${claim.description}</p></div></div><span class="status ${statusClass(claim.status)}">${statusLabel(claim.status)}</span></div>
    ...
    <div class="actions"><p>${claim.status === 'UnderReview' ? 'Analyst decision required to continue this claim.' : 'Claim activity is synchronised across connected systems.'}</p><div class="action-buttons"><button class="action-button reject" data-decision="false" ${claim.status !== 'UnderReview' ? 'disabled' : ''}>Reject claim</button><button class="action-button approve" data-decision="true" ${claim.status !== 'UnderReview' ? 'disabled' : ''}>Approve claim</button></div></div>`;
